...
import { ArrowLeft, Wallet, Zap, Volume2, VolumeX } from "lucide-react"
...
const bloggerConfig = {
  ...
  prizeConfig: {
    small: {
      values: [2, 3, 5, 8], // Prêmios pequenos para bloggers
...
export default function MegaSortePage() {
  ...
  const drawScratchLayer = (ctx: CanvasRenderingContext2D, width: number, height: number, cellIndex: number) => {
    ...
    if (!scratchImageRef.current) {
      // Tema azul/roxo para Mega Sorte
      ...
      ctx.fillText("MEGA", width / 2, height / 2 - 6)
      ctx.fillText("SORTE", width / 2, height / 2 + 6)
  ...
  const highlightWinningCells = () => {
    ...
    gameStateRef.current.scratchCanvases.forEach((canvas, index) => {
      ...
      if (symbolText === winningSymbol) {
        // Aplicar efeitos visuais de destaque - tema azul/roxo
        cellContent.style.animation = "pulse 1.5s ease-in-out infinite"
        cellContent.style.boxShadow = "0 0 25px rgba(99, 102, 241, 0.8), inset 0 0 25px rgba(99, 102, 241, 0.3)"
        cellContent.style.border = "3px solid #6366F1"
        cellContent.style.backgroundColor = "rgba(99, 102, 241, 0.15)"
    ...
    try {
      ...
      for (let i = 0; i < NUM_CELLS; i++) {
        ...
        cellContainer.className =
          "relative w-full pb-[100%] bg-gray-900 rounded-lg overflow-hidden border border-indigo-400/40 shadow-inner"

        const cellContent = document.createElement("div")
        cellContent.className =
          "absolute inset-0 flex flex-col justify-center items-center text-indigo-400 font-bold opacity-0 scale-75 transition-all duration-300 bg-gray-800 rounded-lg p-2"
        ...
        if (symbol.imageUrl) {
          ...
          img.onerror = () => {
            img.src = `https://placehold.co/60x60/4338CA/6366F1?text=${encodeURIComponent(symbolIds[i])}`
          }
      ...
      toast({
        title: "Jogo iniciado!",
        description: "Raspe as células para descobrir sua mega sorte!",
      })
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Link href="/jogos">
                <Button variant="ghost" size="sm" className="text-gray-300 hover:text-indigo-400">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-lg font-bold text-white">Mega Sorte</h1>
            ...
            <div className="flex items-center space-x-3">
              ...
              {userProfile && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-indigo-400 font-bold">R$ {formatCurrency(userProfile.wallet.balance)}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <main className="relative z-10 max-w-2xl mx-auto px-4 py-8">
        <Card className="bg-gray-800/50 border-gray-700 p-6">
          ...
          <div className="text-center mb-6">
            <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-indigo-400 mb-2">
              MEGA SORTE
            </h2>
            <Badge className="bg-indigo-500 text-white font-bold">A sorte da sua vida!</Badge>
              ...
              <Button
                ...
                className={`w-full font-bold py-3 text-lg ${
                  gameStateRef.current.gameEnded && gameStateRef.current.hasWonRealPrize
                    ? "bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white animate-pulse"
          ...
          <div className="mt-8 bg-gray-700/30 rounded-lg p-4">
            <h3 className="text-indigo-400 font-bold mb-3">Como Jogar:</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm text-gray-300">
              <div className="text-center">
                <div className="w-6 h-6 bg-indigo-500 rounded-full flex items-center justify-center text-white font-bold mx-auto mb-2 text-xs">
                  1
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <Card className="bg-gray-800 border-gray-700 p-6 w-full max-w-md">
            <h2 className="text-2xl font-bold text-center mb-4 text-indigo-400">{modalTitle}</h2>
            <p className="text-gray-300 text-center mb-6">{modalMessage}</p>
            <p
              className={`text-2xl font-extrabold text-center mb-4 ${modalType === "win" ? "text-indigo-400" : "text-gray-400"}`}
            >
              {modalTitle}
            </p>
            <Button
